...
class CarDealership(BaseModel):
    ...
    balance = MoneyField(max_digits=14, decimal_places=2, default_currency='USD', verbose_name='Car dealership balance')
    description_cars = models.JSONField(encoder=None, decoder=None, verbose_name='Description future cars to sale')
    dealership_cars = models.ManyToManyField('Dealer.DealerCars', through='CarDealershipDealersCar',
                                             verbose_name='List cars to sale')
    list_models = models.ManyToManyField('Dealer.CarModel', through='CarDealershipModel', verbose_name='Models to sale',
...
class Discount(BaseModel):
    ...
    percent = models.PositiveIntegerField(validators=[MaxValueValidator(50)], verbose_name='Percent discount')
    car_dealership = models.ForeignKey(CarDealership, on_delete=models.SET_NULL, null=True, related_name='discounts',
...
class CarDealershipModel(BaseModel):
    ...
    car_dealership = models.ForeignKey(CarDealership, on_delete=models.SET_NULL, null=True,
                                       related_name='list_car_model', verbose_name='Car dealership')
...
class CarDealershipDealersCar(BaseModel):
    car_dealership = models.ForeignKey(CarDealership, on_delete=models.SET_NULL, null=True, related_name='list_cars',
                                       verbose_name='Car dealership which buy car')
    dealer_car = models.ForeignKey('Dealer.DealerCars', on_delete=models.SET_NULL, null=True, related_name='history',
                                   verbose_name='Dealer car which was bought')
    discount = models.ForeignKey(Discount, on_delete=models.SET_NULL, null=True, verbose_name='Discount for bought')
