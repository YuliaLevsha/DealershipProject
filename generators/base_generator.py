...
class Generator:
    ...
    def create_customer(self):
        ...
        future_customer = Customer.objects.create(username=self.generate_string(), 
    ...
    def create_car(self):
        Car.objects.create(car_model=CarModel.objects.get(pk=car_random.random_car_model()),
    ...
    def generate_dealer_cars(self):
        ...
        DealerCars.objects.create(dealer=Dealer.objects.get(pk=random.randint(1, dealer_count+1)),
                                  car=car,
    ...
    def create_dealer(self):
        Dealer.objects.create(name=self.generate_string(),
                              foundation_year=random.randint(2000, 2024))
    
    def create_dealership(self):
        car_model = CarModel.objects.get(pk=car_random.random_car_model())
        cardealership = CarDealership.objects.create(name=self.generate_string(),
                                                     location=random.choice(G8Countries.only),
                                                     balance=random.randint(100000, 300000),
                                                     description_cars={'car_model': car_model.name,
                                                                       'car_year': car_random.random_car_year(),
                                                                       'car_color': car_random.random_car_color(),
                                                                       'number_of_doors': car_random.random_car_number_of_doors(),
                                                                       'body_type': car_random.random_car_body_type(),
        ...
        AvailableCarModels.objects.create(car_model=car_model,
                                          car_dealership=cardealership)
    
    def create_offer(self):
        car_counts = Car.objects.all().count()
        customer_indexes = Customer.objects.filter(is_active=True).values_list('id', flat=True)
        Offer.objects.create(max_price=random.randint(10000, 50000),
                             customer=Customer.objects.get(pk=random.randint(1, random.choice(customer_indexes))),
