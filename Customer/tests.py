...
class ForgotPasswordTest(TestCase):
    ...
    def test_post(self):
        request = self.factory.post('api/forgot-password/', data={
            'email': EMAIL
        })
        response = self.view(request)
        self.assertEquals(response.status_code, 200)
...
class ChangePasswordTest(TestCase):
    ...
    def test_update(self):
        response = self.client.put('/api/change-password/', data={
...
class UpdateUsernameEmailTest(TestCase):
    ...
    def test_update(self):
        response = self.client.put('/api/update-user/', data={
            'username': 'YuliaL',
            'email': EMAIL
        })
        self.assertEquals(response.status_code, 200)
...
class ConfirmEmailTest(TestCase):
    def setUp(self) -> None:
        self.client = APIClient()
        self.user = Customer.objects.create(username=USER_NAME, email=EMAIL, password=make_password(PASSWORD),
                                            is_active=False)
        self.uidb64 = urlsafe_base64_encode(force_bytes(self.user.pk))
        self.token = account_activation_token.make_token(self.user)

    def test_retrieve(self):
        url = reverse('confirm_email', kwargs={'uidb64': self.uidb64, 'token': self.token})
