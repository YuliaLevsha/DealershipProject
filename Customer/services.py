...
def form_message(request, user, template, action_type):
    ...
    activation_url = reverse_lazy(action_type, kwargs={'uidb64': urlsafe_base64_encode(force_bytes(user.pk)),
...
def send_activation_email(request, user, action_type):
    ...
    """
    message = form_message(request=request, template=MESSAGE_TEMPLATES[action_type]['message'], action_type=action_type,
                           user=user)
    send_message_to_email(subject=MESSAGE_TEMPLATES[action_type]['subject'], message=message, recipient=user.email)
